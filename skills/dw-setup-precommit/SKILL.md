...
description: >-
  ...
  commit` auto-formats and lints the staged files with the tools the project already has. Tracked
  config that runs on every commit, distinct from `.claude/hooks/*.sh` which only run inside a
...
files, every time. The point is to catch unformatted or lint-failing code at the
commit boundary rather than in CI — so the diff that lands is already clean.

This is **git-level config**: `.husky/pre-commit` and the lint-staged map are
committed to the repo and run on **every** `git commit`. That is deliberately
**distinct from `.claude/hooks/*.sh`** — those only fire inside a Claude Code
session. The gap is the reason this skill is worth running even in a repo only you
touch: the commits you make by hand, from another editor, or in a hurry get the
same check as the ones an agent makes. A repo can have both; this skill installs
the half that doesn't depend on how the commit was authored.
...
All of these are **tracked in git** — that's the whole point: they travel with the
repo, so a fresh clone and CI get the same check. Nothing here is personal/ignored.
...
### 3. HARD STOP gate — it changes every commit, ask first

Before installing anything, stop and lay out the plan for explicit consent.
State plainly: **this makes every `git commit` in the repo slower and able to
fail** — including hand-made commits outside any agent session. Show what will be
installed, the exact `.husky/pre-commit` and `.lintstagedrc.json` you'll write
(from the detection in step 2), and wait.
...
hook, so a stray `npm install` can't quietly produce a second lockfile:
...
were wired and which were skipped (and why). Be specific — this config now runs on
every commit, so it should be no surprise later what it does.
...
- **HARD STOP before install.** This changes every commit in the repo, including
  ones made outside a session (step 3) — explicit consent first, always.
