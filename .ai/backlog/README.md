...
**Eight entries, and `validate-artifacts.sh` enforces it** (this README doesn't count). A ninth means
one of three things has to happen first: bundle it with a cousin that ships alongside it, absorb the
cheapest entry into the change that is open, or admit an old one failed the month bar and `git rm` it.
The cap exists because a backlog nobody ever deletes from stops being a queue and becomes a record of
