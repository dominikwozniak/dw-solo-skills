...
- [ ] 1. `skills/dw-next/SKILL.md`: modes — bare = report, then build all remaining tasks; `status` = report and stop; `go` = one task; new argument-hint; unchanged: one commit per task, stop at a decision or an irreversible step.
