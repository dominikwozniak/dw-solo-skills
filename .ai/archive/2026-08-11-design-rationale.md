...
- **One change is one independently shippable scope** — the question "could each piece land on its own
  and leave the repo green?", asked at **shape time** rather than discovered mid-build. A request
  ...
  **ordering** sentence in the `## Notes` of whichever lands second — never a dependency field, which
  would be the status column this lane exists to avoid.
