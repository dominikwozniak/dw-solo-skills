...
Why this repo is shaped the way it is. `dw-land` promotes a record here at the end of a change;
`dw-shape` reads the folder before shaping the next one. Durable — nothing here expires with the
change that wrote it.

This is the same contract `templates/decisions-README.md` ships into a scaffolded repo, kept as a
hand-written copy rather than a symlink: `templates/` is payload copied verbatim into a target
project, and the only symlink to it is `plugins/dw-solo-setup/templates`. If the contract changes,
it changes in both.
...
What a word _means_ goes in `CONTEXT.md`, one line, no rationale. A trap this repo has actually
sprung goes in `## Gotchas` in `AGENTS.md` (which `CLAUDE.md` symlinks to). Both are common; a
decision record is rare.
