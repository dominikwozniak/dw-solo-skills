...
- `.claude/settings.json` — parsed for every wired hook command; each referenced
  `*.sh` is checked for existence + the executable bit. Plus `enabledPlugins`, to
  confirm the team-lane plugins really are disabled here — a hand-written key with
  a wrong marketplace id is silently ignored, so it's worth asserting.
- `.ai/work/` — this lane's home, and the check that the repo is scaffolded.
  `.ai/runs/` is the **team** lane's; finding it here means the wrong plugin is
  installed for this repo, so it's reported rather than ignored.
- `docs/decisions/`, `CONTEXT.md`, and `CLAUDE.md`'s `## Gotchas` / `## Commands`
  sections — the four targets `dw-land` promotes into, so their absence breaks the
  closing step.
...
- **`.ai/work/` present** — `dw-init` if the scaffold is incomplete (a warned
  `docs/decisions/`, `CONTEXT.md`, or `## Gotchas`), else `dw-shape` for a new change
  or `dw-next` to pick the active one back up.
- **`.ai/work/` absent** — the repo isn't set up for this lane yet: `dw-init`.
- **`.ai/runs/` reported** — this repo runs the team lane, so `dw-solo` is the wrong
  plugin for it; install `dw-planning` + `dw-quality` from the `dw-skills`
  marketplace instead.
