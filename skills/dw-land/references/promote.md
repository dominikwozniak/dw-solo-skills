...
- **Promote the gotchas.** A trap that cost real time, or repeated, becomes one entry — the local
  ...
  **routed topic file** whose subject covers the trap: find the `## Task Router` row that matches and
  ...
  - **Ask first whether a mechanism would catch it.** A trap a hook, a validator, a self-test or a lint
    ...
    and a rule enforced on trust is one a tired session skips. Where a mechanism would work, it takes
    the same tiers as a follow-up: session-sized → **build it before closing**; bigger → one
    `.ai/backlog/<date>-<slug>.md` in the full format naming it. Either way **no `## Gotchas` entry**; a trap written up
...
- **Promote the follow-ups — after doing the ones that don't deserve to be one.** Take every
  ...
  **should it have been done now?** first: nothing blocks it and doing it costs no more than
  describing it → **do it before closing**, an ordinary commit ahead of this one, and it never
  becomes a file. What survives that test is two-tier. **Report-open** is the default: a one-line
  open item carried in the land report and the PR body — no file, no residue. A
  `.ai/backlog/<YYYY-MM-DD>-<slug>.md` file is only for work that genuinely exceeds the session —
  ...
  specific description — **never overwrite it silently**; the existing entry is queued work. Zero files is the normal
