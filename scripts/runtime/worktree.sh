...
"
  ...
  if [ -d "$dst_root/.husky" ] && [ ! -d "$dst_root/.husky/_" ]; then
    missing="$missing  - COMMIT HOOKS ARE INACTIVE — .husky/_/ is generated by the install and gitignored, so git finds no hooks here and every commit skips the pre-commit gate silently. The install above fixes it.
"
  fi
